...
#define PATH_BATTERY_STATUS "/sys/class/power_supply/" BATTERY "/status"
#define DATETIME_FORMAT "  %d.%m.%Y %a %H:%M"
#define SLEEP_SEC 5
...
static int  capacity_int;
static char battery_status[32];
static char lnk[32];
static char ssid[32];
static char datetime[32];
static char cpu_load[32];
static char mem_usage[32];
static char buf[32 +
                sizeof layout +
                sizeof volume +
                sizeof capacity +
...
static void getkblayout(void)
{
    ...
    do
    {
        ...
        if (state_r->lockedMods & XCB_MOD_MASK_LOCK)
            ...
            layout[name_length] = (char)tolower(name[name_length]);
    } while (name_length);
...
int main(void)
{
    ...
    for (;; nanosleep(&ts, NULL))
    {
        ...
        if (!(--count_1))
        {
            ...
            fp = popen("iw dev " IFACE " info | grep ssid | awk '{print $2}'", "r");
            if (fp == NULL) ssid[0] = '\0';
            else if (fgets(ssid, sizeof(ssid), fp) != NULL)
                ssid[strcspn(ssid, "\n" )] = '\0';
            pclose(fp);



	    fp = popen("top -b -n1 | grep 'Cpu(s)' | awk '{print $2 + $4}'", "r");
            if (fp == NULL) cpu_load[0] = '\0';
            else if (fgets(cpu_load, sizeof(cpu_load), fp) != NULL)
                cpu_load[strcspn(cpu_load, "\n" )] = '\0';
            pclose(fp);

	    fp = popen("free -h | sed 's/,/./g' | sed 's/Mi/M/g' | sed 's/Gi/G/g' | awk '/^Mem:/ {print $3 \"/\" $2}'", "r");
            if (fp == NULL) mem_usage[0] = '\0';
            else if (fgets(mem_usage, sizeof(mem_usage), fp) != NULL)
                mem_usage[strcspn(mem_usage, "\n" )] = '\0';

  	    

	    if ((fd = open(PATH_BATTERY_STATUS, O_RDONLY)) != -1)
            {
                if ((n = read(fd, battery_status, sizeof battery_status)) > 0) {
                    battery_status[--n] = '\0';
                    if (strcmp(battery_status, "Charging") == 0) {
                        strcpy(battery_status, "   ");
                    } else {
                        if (capacity_int > 95)
                            strcpy(battery_status, "   ");
                        else if (capacity_int > 60)
                            strcpy(battery_status, "   ");
                        else if (capacity_int > 40)
                            strcpy(battery_status, "   ");
